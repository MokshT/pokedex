...
document.addEventListener("DOMContentLoaded", () => {
  MAX_POKEMONS = 151;
  pokemonID = new URLSearchParams(window.location.search).get("id");
...
async function loadPokemon(id) {
  try {
    ...
    if (currentPokemonId === id) {
      ...
      window.history.pushState({}, "", `./detail.hmtl?id=${id}`);
    }
...
function setTypeBackgroundColor(pokemon) {
  ...
  const styleTang = document.createElement("style");
  styleTang.innerHTML = `
    ...
    `;
  document.head.appendChild(styleTang);
}
...
function displayPokemonDetails(pokemon) {
  ...
  const capitalizePokemonName = capitalizeFirstLetters(name);
